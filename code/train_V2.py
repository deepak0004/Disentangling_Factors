...
class Train:
	...
	def __init__(self, model, dataset):
		...
		self.no_iters = 500
		self.batch_size = 16
		self.weights_dir = "weights_svae2/model.ckpt"
		self.test_dir = "weights_svae2/model.ckpt-39"
		
		self.reuse_weights = False
		self.retrain_dir = "weights_best_svae/model.ckpt-17"
		...
		self.recweight = 0.6
		self.swap1weight = 1
		self.swap2weight = 0.1
		self.classweight = 1
		self.klweightZ = 0.1
		self.klweightS = 0.1
		self.samplingweight = 0.02
		...
		self.sample_s = self.getSample(self.s, (self.batch_size*3, self.sSize), self.sSize)
		...
		self.sample_z32 = self.getSample(self.z32, (self.batch_size*2, self.zSize))
		...
		self.sample_s23 = self.getSample(self.s23, (self.batch_size*2, self.sSize), self.sSize)

		self.s_21_23 = tf.concat([self.s, self.s21, self.s23], axis=0)

		self.sample_s_21_23 = self.getSample(self.s_21_23, (self.batch_size*7, self.sSize), self.sSize)

		self.y_rec_swap1_swap2 = model.getDecoder(self.sample_z_12_32, self.sample_s_21_23, self.nFeatures, self.zSize, self.sSize, False, False, self.nChannels, self.hInput, self.wInput)
		...
		self.z_swap2_normal = tf.random_normal(shape=[self.batch_size*2, self.zSize])
		self.y_swap2_normal = model.getDecoder(self.z_swap2_normal, self.sample_s23, self.nFeatures, self.zSize, self.sSize, False, False, self.nChannels, self.hInput, self.wInput)
		...
		self.s_normal2 = tf.random_normal(shape=[self.batch_size*2, self.sSize])
		self.y_swap2_normal2 = model.getDecoder(self.sample_z32, self.s_normal2, self.nFeatures, self.zSize, self.sSize, False, False, self.nChannels, self.hInput, self.wInput)
		self.dis_y_normal2 = model.getDiscriminator(self.y_swap2_normal2, self.sid23, self.nFeatures, self.nSid, self.nChannels, self.hInput + 4, self.wInput + 4)
		#self.loss_gen += self.samplingweight * tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(labels=self.sids_real, logits=self.dis_y_normal))
		self.loss_gen += self.samplingweight * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=sids_real, logits=self.dis_y_normal2))
		...
		#KL Divergence Loss
		z = tf.unstack(self.s, axis=1)
		z_mu = z[0]
		z_sigma = z[1]
		self.loss_s1 = tf.reduce_mean(-0.5 * tf.reduce_sum(1.0 + tf.log(tf.square(z_sigma)) - tf.square(z_mu) - tf.exp(tf.log(tf.square(z_sigma))), 1))
		klweightS = self.klweightS*6/(self.nChannels*self.hInput*self.wInput)
		self.loss_gen += klweightS * self.loss_s1
	...
	def startTesting(self):
		...
		with tf.Session(config = self.config) as sess:
			...
			y_swap1, y_swap2, y_rec = sess.run([self.y_swap1, self.y_swap2, self.y_rec], feed_dict = f_dict)
			z, s, yy = sess.run([self.sample_z, self.sample_s, self.sids], feed_dict=f_dict)
			...
			self.draw(y_rec[0,0,:,:], "y_rec1.png")
			self.draw(y_rec[self.batch_size*1,0,:,:], "y_rec2.png")
			self.draw(y_rec[self.batch_size*2,0,:,:], "y_rec3.png")
	...
	def dump_latent_space(self, z, s, y):
		pickle.dump(z, open("latent_space_dumps_svae/label_space_z.dump", "w"))
		pickle.dump(s, open("latent_space_dumps_svae/label_space_s.dump", "w"))
		pickle.dump(y, open("latent_space_dumps_svae/labels.dump", "w"))
	...
	def draw(self, data, name="test.png"):
		...
		img = toimage(temp)
		img.save('image_dumps_svae2/' + name)
		print('image_dumps_svae2/' + name)
