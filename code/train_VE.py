...
class Train:
	...
	def __init__(self, model, dataset):
		...
		self.no_iters = 500
		self.batch_size = 16
		self.weights_dir = "weights_svae_vgg/model.ckpt"
		self.test_dir = "weights_svae_vgg/model.ckpt-24"
		
		self.reuse_weights = False
		self.retrain_dir = "weights_svae_vgg/model.ckpt-17"
		...
		self.dis_yswap2 = model.getDiscriminatorVGG(self.y_swap2, images23, self.nFeatures, self.nSid, self.nChannels, self.hInput + 4, self.wInput + 4)
		...
		self.dis_y_normal = model.getDiscriminatorVGG(self.y_swap2_normal, images23, self.nFeatures, self.nSid, self.nChannels, self.hInput + 4, self.wInput + 4)
		...
		self.dis_y_normal2 = model.getDiscriminatorVGG(self.y_swap2_normal2, images23, self.nFeatures, self.nSid, self.nChannels, self.hInput + 4, self.wInput + 4)
		#self.loss_gen += self.samplingweight * tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(labels=self.sids_real, logits=self.dis_y_normal))
		...
		self.dis_y_rec_23 = model.getDiscriminatorVGG(images23, images23, self.nFeatures, self.nSid, self.nChannels, self.hInput + 4, self.wInput + 4)
	...
	def draw(self, data, name="test.png"):
		...
		img = toimage(temp)
		img.save('images_dumps_vgg/' + name)
		print('images_dumps_vgg/' + name)
